...
const LicenciasMedicas: React.FC<ILicenciasMedicasProps> = () => {
  ...
  const differenceInDays = (startDate: Date, endDate: Date): number => {
    ...
    const diffMilliseconds = utcEndDate - utcStartDate;
    console.log(Math.floor(diffMilliseconds / (1000 * 60 * 60 * 24)))
  ...
  const handleInputChange = (value: any, stateName: string) => {
    ...
    if (selectedEmployee) {
      ...
      if (stateName === 'EndDate') {
        const { DepartureDate } = registerLeave
        setRegisterLeave('TotalDays', differenceInDays(DepartureDate, value))
      }
  ...
  return (
    ...
    <>
      ...
      <div className='my-4 flex justify-end gap-4'>
        <LicenciasMedicasButtonComponent onClick={sendRegisterLeave} buttonName='Guardar' disabled={Object.keys(validationMessage).length > 0} buttonStyle='py-[6px] hover:cursor-pointer border-0 rounded-sm px-5 text-sm bg-sky-700 text-white font-bold' />
